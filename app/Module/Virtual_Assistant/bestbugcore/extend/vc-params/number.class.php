...
array(
	'type'       => 'number',
	'heading'    => esc_html__( 'Number field', 'virtual-assistant' ),
...
if(!class_exists('Exlac_Extend_VcParams_Number'))
{
	class Exlac_Extend_VcParams_Number
	{
		...
		function init()
		{
			if ( class_exists( 'WpbakeryShortcodeParams' ) && is_admin() )
			{
				// Load enqueueScripts
				if(is_admin()) {
					WpbakeryShortcodeParams::addField('bb_number' , array($this, 'bb_number'), EXLAC_CORE_URL . '/assets/admin/js/extend/vc-params/number.js');
	...
	new Exlac_Extend_VcParams_Number();
}
