...
if (!class_exists('Exlac_Helper')) {
	...
	class Exlac_Helper
	{
		...
		*/
		...
		{
			$_GET = Exlac_Helper::sanitize_data( $_GET );
			?>
	...
	{
		$dev = isset( $_COOKIE['dev'] ) ? sanitize_key( wp_unslash( $_COOKIE['dev'] ) ) : '';
		$sync = isset( $_COOKIE['sync'] ) ? sanitize_key( wp_unslash( $_COOKIE['sync'] ) ) : '';
		$port = isset( $_COOKIE['port'] ) ? sanitize_key( wp_unslash( $_COOKIE['port'] ) ) : '';
		$version = isset( $_COOKIE['version'] ) ? sanitize_key( wp_unslash( $_COOKIE['version'] ) ) : '2.14.0';
		if ( $dev == 1) {
			if ( $sync != '') {
				$src = $sync;
				?><script id="__bs_script__">//<![CDATA[
					...
					$src = $port ? 'http://HOST:' . $port : 'http://HOST:3000';
					?><script id="__bs_script__">//<![CDATA[
